import { base64StringToBlobOrBuffer, btoa, readAsBinaryString, atob, binaryStringToBlobOrBuffer, blobOrBufferToBase64 } from 'pouchdb-binary-utils';
import { isLocalId, checkBlobSupport, parseDoc } from 'pouchdb-adapter-utils';
...
function processAttachment(name, src, doc, isBinary, attachmentFormat) {
  ...
  if (attachmentFormat === 'base64') {
    if (isBinary) {
      ...
      doc._attachments[name].data = base64StringToBlobOrBuffer(att.data, att.content_type);
    } else {
...
function getAttachment(docId, attachId, attachment, opts, cb) {
  ...
  if (typeof data === 'string') {
    if (opts.binary) {
      cb(null, base64StringToBlobOrBuffer(data, attachment.content_type));
    } else {
      cb(null, data);
    }
    return;
    ...
    if (typeof attachment.data === 'string') {
      ...
      if (metadata.idb_attachment_format === 'base64') {
        ...
        return new Promise(resolve => {
          blobOrBufferToBase64(attachment.data, function (b64) {
            attachment.data = b64;
